      ...
      {!loading && (
        <div className="relative">
          ...
          <div
            ...
            style={{ overflowX: 'hidden', paddingLeft: '2rem', paddingRight: '2rem' }}
            onMouseEnter={() => { paused.current = true }}
            onMouseLeave={() => { paused.current = false }}
          >
            {[...starred, ...starred].map((repo, i) => {
              const idx = i % starred.length
              const langColor = langColors[repo.language ?? ''] ?? '#525252'
              const bg = langGradients[repo.language ?? ''] ?? 'linear-gradient(135deg, #0f0f0f 0%, #1a1010 100%)'
                ...
                >
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <span className="font-mono text-[9px] text-[#525252] uppercase tracking-wider">★ STARRED</span>
                      <span className="match-badge text-[11px] flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#4ade80]" />
                        {Math.max(79, 95 - idx)}% MATCH
                      </span>
                    </div>
